...
public class PickImageHelper {
     ...
     * 打开图片选择器
     */
    public static void pickImage2(final Context context, final int requestCode, final PickImageOption option, PickImageAction.ImageActionType imageActionType) {
        switch (imageActionType) {
            case PHOTO:
                int from2 = PickImageActivity.FROM_LOCAL;
                if (!option.crop) {
                    PickImageActivity.start((Activity) context, requestCode, from2, option.outputPath, option.multiSelect,
                            option.multiSelectMaxCount, true, false, 0, 0);
                } else {
                    PickImageActivity.start((Activity) context, requestCode, from2, option.outputPath, false, 1,
                            false, true, option.cropOutputImageWidth, option.cropOutputImageHeight);
                }
                break;
            case CAMERA:
