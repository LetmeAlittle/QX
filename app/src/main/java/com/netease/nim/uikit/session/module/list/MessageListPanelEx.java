...
public class MessageListPanelEx {
    ...
    private OnItemClickListener listener = new OnItemClickListener() {
        @Override
        public void onItemClick(IRecyclerView adapter2, View view, int position) {
            IMMessage imMessage = adapter.getItem(position);
            String account = imMessage.getFromAccount();
            if (login) {
//                //将当前用户id传递过去  启动UserMainActivity
//                Intent intent = new Intent(container.activity, UserMainActivity.class);
//                intent.putExtra("id", Integer.parseInt(account));//用户id
//                intent.putExtra("accid", Integer.parseInt(account));//网易accid
//                container.activity.startActivity(intent);
            } else {
                IntentUtil.jumpIntent(container.activity, NLoginActivity.class);
            }
        }
