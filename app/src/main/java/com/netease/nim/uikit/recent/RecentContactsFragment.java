...
public class RecentContactsFragment extends TFragment {
	...
	{
		touchListener = new SimpleClickListener<RecentContactAdapter>() {
			@Override
...
//            items.addAll(loadedRecents);  TODO：过去群聊消息
			int size = loadedRecents.size();
			for (int i = 0; i < size; i++) {
				if (loadedRecents.get(i).getSessionType() == SessionTypeEnum.P2P) {
					items.add(loadedRecents.get(i));
				}
			}
			loadedRecents = null;
		}
	...
	private void onRecentContactChanged(List<RecentContact> recentContacts) {
		...
		for (RecentContact r : recentContacts) {
			...
			if (r.getSessionType() == SessionTypeEnum.P2P) {//TODO: update
				items.add(r);
			}
