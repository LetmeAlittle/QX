...
public class ShouHuSearchAdapter extends BaseQuickAdapter<SearchListResponse.DataBean.ListBean, BaseViewHolder>{

    private int time;

    public ShouHuSearchAdapter(int layoutResId, int time) {
        super(layoutResId);
        this.time = time;
    }
    ...
    protected void convert(BaseViewHolder helper, SearchListResponse.DataBean.ListBean item) {
        ...
        if (isOnLine == 0) {
            ...
            tvChoose.setOnClickListener(new ChooseClick("" + item.getId(), item.getNick_name(), item.getMember_avatar()));
        }
