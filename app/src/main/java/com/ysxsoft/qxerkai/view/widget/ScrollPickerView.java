...
public abstract class ScrollPickerView<T> extends View {
    ...
    private int mVisibleItemCount = 9; // 可见的item数量

    private boolean mIsInertiaScroll = true; // 快速滑动时是否惯性滚动一段距离，默认开启
    private boolean mIsCirculation = false; // 是否循环滚动，默认开启
    ...
    private int mSelected = 0; // 当前选中的item下标
    ...
    private boolean mDrawAllItem = true; // 是否绘制每个item(包括在边界外的item)
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isFirst) {//第一次快速滑动到左边
            isFirst = false;
//            autoScrollFast(0,100);
            mSelected = 0;
        }
