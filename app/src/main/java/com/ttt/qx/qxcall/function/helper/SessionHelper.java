package com.ttt.qx.qxcall.function.helper;
...
import com.ttt.qx.qxcall.function.voice.AVChatAction;
...
import com.ttt.qx.qxcall.function.voice.MsgViewHolderAVChat;
    ...
    private static void setSessionListener() {
        SessionEventListener listener = new SessionEventListener() {
            ...
            public void onAvatarClicked(Context context, IMMessage message) {
                ...
                // 一般用于打开用户资料页面
                if (msgType == MsgTypeEnum.robot && message.getDirect() == MsgDirectionEnum.In) {
                    RobotAttachment attachment = (RobotAttachment) msgAttachment;
                    if (attachment.isRobotSend()) {
//                        RobotProfileActivity.start(context, account);
                        return;
                    }
                }
//                UserProfileActivity.start(context,account);
                //如果是音视频消息类型 点击 给出提示 是否发起语音通话
                if (msgType == MsgTypeEnum.audio) {
                    //调起拨打界面。
                    AVChatProfile.getInstance().setAVChatting(true);
                    AVChatActivity.launch(getApplication(), account, AVChatType.AUDIO.getValue(), AVChatActivity.FROM_INTERNAL);
                }
            }
