...
import com.ttt.qx.qxcall.adapter.MentionRecordListAdapter;
...
public class MentionRecordActivity extends BaseActivity {
    ...
    private MentionRecordListAdapter mAdapter;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_mention_record);
    ...
    private void initData() {
        ...
        MineModel.getMineModel().crashList(new ProgressSubscribe<>(new SubScribeOnNextListener<MentionRecordList>() {
            @Override
            public void onNext(MentionRecordList recordList) {
                if (recordList.getStatus_code() == 200) {
                    MentionRecordList.DataBeanX data = recordList.getData();
                    List<MentionRecordList.DataBeanX.DataBean> dataBeanList = data.getData();
                    if (dataBeanList == null) {
                        dataBeanList = new ArrayList<MentionRecordList.DataBeanX.DataBean>();
                    }
                    mAdapter = new MentionRecordListAdapter(mContext, dataBeanList);
    ...
    private void loadMore() {
        ...
        if (mCurrentPage < mLastPage) {
            ...
            MineModel.getMineModel().crashList(new ProgressSubscribe<>(new SubScribeOnNextListener<MentionRecordList>() {
                @Override
                public void onNext(MentionRecordList list) throws IOException {
                    if (list.getStatus_code() == 200) {
                        MentionRecordList.DataBeanX data = list.getData();
                        List<MentionRecordList.DataBeanX.DataBean> dataBeanList = data.getData();
                        if (dataBeanList == null) {
                            dataBeanList = new ArrayList<MentionRecordList.DataBeanX.DataBean>();
                        }
