...
//        avChatVideo = new AVChatVideo(context, root.findViewById(R.id.avchat_video_layout), this, this);
        avChatAudio = new AVChatAudio(context, root.findViewById(R.id.avchat_audio_id), this, this);
        avChatSurface = new AVChatSurface(context, this, root.findViewById(R.id.avchat_surface_id), cb);
//        avChatAudio = new AVChatAudio(context, View.inflate(context,R.layout.avchat_audio_layout,null), this, this);
//        avChatSurface = new AVChatSurface(context, this, View.inflate(context,R.layout.avchat_surface_layout,null),cb);

        return true;
    }

    public Chronometer getTime() {
        return avChatAudio.getTime();
    }
    /**
     * ******************************拨打和接听***************************
     */
...
//                }
    ...
    private static final int INCOMING_SUCCESS = 8;
            ...
            switch (msg.what) {
                case SUCCESS:
                    ...
                    if (data.getIs_allow() == 1) {//不处理
                        //设置当前剩余金币数
                        if (avChatAudio != null) {
                            avChatAudio.setGoldNum(String.valueOf(data.getMember_account()));
                        }
                    } else {
                        //直接挂断通话
                        onHangUp();
                ...
                case INCOMING_SUCCESS:
                    String goldNum2 = (String) msg.obj;
                    if (avChatAudio != null) {
                        avChatAudio.setGoldNum(goldNum2);
                    }
                    break;
                case SET_GOLD_NUM:
                    String goldNum = (String) msg.obj;
                    avChatAudio.setGoldNum(goldNum);
                    break;
            }
        }
    };
...
//                // 关闭音频
//                AVChatManager.getInstance().muteLocalAudio(true); //关闭本地流 让对方听不到自己的声音
//            } else {
//                // 打开音频
...
//            }
            ...
            if (!AVChatManager.getInstance().isRemoteAudioMuted(receiverId)) { // isMute是否处于静音状态
                // 关闭音频
                AVChatManager.getInstance().muteRemoteAudio(receiverId, true); //关闭远端流 不听对方的声音
            } else {
                // 打开音频
        ...
        FindModel.getFindModel().getGiftList(new ProgressSubscribe<>(new SubScribeOnNextListener<GiftList>() {
            ...
            public void onNext(GiftList giftList) {
                if (giftList.getStatus_code() == 200) {
                    GiftSendDialog.showBottomDialog((AVChatActivity) context, giftList.getData(), new GiftSendDialog.OnComponentClickListener() {
                        @Override
                        ...
                        public void onSend(String gift_id) {
                            ...
                            }, context), gift_id, receiverId, mAuthorization);
                        }
                    });
                } else {
