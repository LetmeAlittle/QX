...
public class HomeModel implements BaseModel {
	 ...
	 * @param Authorization
	 */
	public void getZhiLiaoInfo(Subscriber<UserDetailInfo> subscriber, String id, String Authorization) {
		Observable<UserDetailInfo> observable = homeServiceApi.getZhiLiao(id, Authorization);
	 ...
	 * @param subscriber 观察者对象
	 */
	private void onToSubscribe89(Observable<UserDetailInfo> observable, Subscriber<UserDetailInfo> subscriber) {
		subscription = observable.subscribeOn(Schedulers.io())
				.unsubscribeOn(Schedulers.io())
				.observeOn(AndroidSchedulers.mainThread())
				.subscribe(subscriber);
